...
unsigned int ui_render_scroll_list(struct ui_scroll_list*         thiz,
                                   unsigned int                   select,
                                   unsigned int                   count,
                                   struct ui_geometry const*      geometry,
                                   struct ui_scroll_list_refresh* refresh)
{
	...
	ut_assert(geometry);
	ut_assert(refresh);
	ut_assert(select < count);

	if (select < thiz->scrl_top) {
		refresh->scrl_top = select;
		thiz->scrl_top = select;
		goto full;
	}

	if (select >= (thiz->scrl_top + geometry->h)) {
		refresh->scrl_top = select + 1 - geometry->h;
		thiz->scrl_top = refresh->scrl_top;
		goto full;
	}

	if (geometry->w != thiz->scrl_cols) {
		refresh->scrl_top = thiz->scrl_top;
		goto full;
	}

	if (geometry->h <= thiz->scrl_rows) {
		thiz->scrl_rows = ut_min(thiz->scrl_rows, geometry->h);
		refresh->scrl_cnt = 0;
		goto out;
	}

	refresh->scrl_top = thiz->scrl_top + thiz->scrl_rows;
	refresh->scrl_row = thiz->scrl_rows;
	refresh->scrl_cnt = ut_min(count - thiz->scrl_top, geometry->h) -
	                   thiz->scrl_rows;
	thiz->scrl_rows += refresh->scrl_cnt;
	goto out;

full:
	refresh->scrl_row = 0;
	refresh->scrl_cnt = ut_min(count - refresh->scrl_top, geometry->h);
	thiz->scrl_rows = refresh->scrl_cnt;

out:
	thiz->scrl_cols = geometry->w;

	return select - thiz->scrl_top;
}
